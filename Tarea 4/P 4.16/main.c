...
temperaturas en el exterior en un perídodo de 24 horas. Calcula el
...
int HMAX;
int HMIN;
/* Variable globales. ACT se utiliza para acumular las temperaturas,
por esa razón se inicializa en cero. MAX se utiliza para calcular la
máxima; se inicializa en -50 para que el primer valor que se inrese
...
void main(void)
{
    ...
    printf("\nMinima del dia: %5.2 \tHora: %d", MIN, HMIN);
}
...
produjo en las variables lobales MAX y HMAX, respectivamente. */
{
    if (MAX < T)
        {
            MAX = T;
...
produjo en las variables globables MIN y HMIN. */
{
    if (MIN > T)
        {
            MIN = T;
